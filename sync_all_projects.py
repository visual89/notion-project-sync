...
def convert_property_for_update(source_prop, target_prop_type):
    ...
    if source_type == "number" and target_prop_type == "number":
        return {"number": source_prop.get("number")}

    if source_type == "select" and target_prop_type == "select":
        value = source_prop.get("select")
        return {"select": {"name": value.get("name")}} if value else {"select": None}
    ...
    if last_edited_time_prop_name:
        ...
        if target_type == "date" and source_last_edited_time:
            desired_props[last_edited_time_prop_name] = {
                "date": {"start": source_last_edited_time}
            }
...
def build_result_properties(row, result_schema):
    ...
    if "No" in result_schema:
        props["No"] = {"number": safe_int(row.get("no"))}

    if "날짜" in result_schema:
        props["날짜"] = {"date": {"start": today_kst_date()}}

    if "팀명" in result_schema:
        props["팀명"] = {
            "title": [{"text": {"content": str(row.get("team_name") or "")}}]
        }

    if "프로젝트 수" in result_schema:
        props["프로젝트 수"] = {"number": safe_int(row.get("count"))}

    if "추가" in result_schema:
        props["추가"] = {"number": safe_int(row.get("added"))}

    if "수정" in result_schema:
        props["수정"] = {"number": safe_int(row.get("updated"))}

    if "미반영" in result_schema:
        props["미반영"] = {"number": safe_int(row.get("skipped"))}

    if "삭제" in result_schema:
        props["삭제"] = {"number": safe_int(row.get("deleted"))}

    if "최종 편집 일시" in result_schema and row.get("latest_edited_time"):
        props["최종 편집 일시"] = {
            "date": {"start": row.get("latest_edited_time")}
        }
...
def make_result_table_text(result_rows):
    lines = []

    lines.append("======================================")
    lines.append("프로젝트 통합 결과")
    lines.append("======================================")
    lines.append("No | 팀명 | 프로젝트 수 | 추가 | 수정 | 미반영 | 삭제 | 최신 편집 일시")
    lines.append("--------------------------------------")
...
def make_report_children(summary_text, detail_text):
    children = [
        ...
        },
        {
            ...
            "paragraph": {
                "rich_text": [
                    {"type": "text", "text": {"content": summary_text[:1900]}}
                ]
            }
        },
        {
...
def main():
    ...
    try:
        ...
        # 성공/확인필요일 때는 노션 실행결과 페이지에 표만 기록
        ...
        # 실패 시에는 원인 확인을 위해 전체 로그와 traceback 기록
        detail_text = "\n".join(REPORT_LINES) + "\n\n" + error_text
...
if __name__ == "__main__":
    main()    WARNING_LINES.append(line)
